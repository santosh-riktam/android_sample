...
package com.riktamtech.android.sellabike.inapp;
...
import com.riktamtech.android.sellabike.inapp.Security.VerifiedPurchase;

/**
 * This class sends messages to Android Market on behalf of the application by connecting (binding) to the MarketBillingService. The application creates an instance of this class
 * and invokes billing requests through this service.
 * 
...
public class BillingService extends Service implements ServiceConnection {
	...
	/**
	 * The list of requests that are pending while we are waiting for the connection to the MarketBillingService to be established.
	 */
	private static LinkedList<BillingRequest> mPendingRequests = new LinkedList<BillingRequest>();

	/**
	 * The list of requests that we have sent to Android Market but for which we have not yet received a response code. The HashMap is indexed by the request Id that each request
	 * receives when it executes.
	 */
	 ...
	 * connected to the MarketBillingService, then we call the run() method directly. Otherwise, we bind to the service and save the request on a queue to be run later when the
	...
	abstract class BillingRequest {
		...
		/**
		 * Called when a remote exception occurs while trying to execute the {@link #run()} method. The derived class can override this to execute exception-handling code.
		 * 
		 * @param e
		 *            the exception
		 */
	...
	/**
	 * The {@link BillingReceiver} sends messages to this service using intents. Each intent has an action and some extra arguments specific to that action.
	 * 
	 * @param intent
	...
	/**
	 * Requests that the given item be offered to the user for purchase. When the purchase succeeds (or is canceled) the {@link BillingReceiver} receives an intent with the action
	 * {@link Consts#ACTION_NOTIFY}. Returns false if there was an error trying to connect to Android Market.
	 * 
	 * @param productId
	...
	/**
	 * Requests transaction information for all managed items. Call this only when the application is first installed or after a database wipe. Do NOT call this every time the
	...
	/**
	 * Confirms receipt of a purchase state change. Each {@code notifyId} is an opaque identifier that came from the server. This method sends those identifiers back to the
	 * MarketBillingService, which ACKs them to the server. Returns false if there was an error trying to connect to the MarketBillingService.
	 * 
	 * @param startId
	 *            an identifier for the invocation instance of this service
	 * @param notifyIds
	...
	/**
	 * Gets the purchase information. This message includes a list of notification IDs sent to us by Android Market, which we include in our request. The server responds with the
	 * purchase information, encoded as a JSON string, and sends that to the {@link BillingReceiver} in an intent with the action {@link Consts#ACTION_PURCHASE_STATE_CHANGED}.
	...
	/**
	 * Verifies that the data was signed with the given signature, and calls {@link ResponseHandler#purchaseResponse(Context, PurchaseState, String, String, long)} for each
	 * verified purchase.
	 * 
	 * @param startId
	 *            an identifier for the invocation instance of this service
	...
	/**
	 * This is called when we receive a response code from Android Market for a request that we made. This is used for reporting various errors and for acknowledging that an order
	 * was sent to the server. This is NOT used for any purchase state changes. All purchase state changes are received in the {@link BillingReceiver} and passed to this service,
	 * where they are handled in {@link #purchaseStateChanged(int, String, String)}.
	 * 
	 * @param requestId
